...
#                 continue
                ...
                end_time = get_time(index, sample_rate, start_time)
            elif current_subtitle != row.prediction:
                start_time = get_time(index, sample_rate, start_time)
                subtitles += f'{subtitle_count}\n'
                subtitles += second2timecode(start_time) + ' --> ' + second2timecode(end_time) + '\n'
